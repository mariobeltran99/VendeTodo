...
@section('container')
    ...
    <div class="flex flex-wrap justify-center">
        <form id="form" enctype="multipart/form-data" class="bg-white items-center w-full shadow-md rounded px-6 mt-6 md:ml-6 lg:ml-6 xl:ml-6 md:mr-6 lg:mr-6 xl:mr-6 mb-6" method="POST" action="/editoProducto" >
            <br>
            @csrf
            <input type="hidden" name="_id" value="{{$listp->id}}">
            ...
            <div class="mb-4">
                ...
                <input
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        name="price" id="price" type="number" min="0.01" step="0.01" placeholder="Ingresa el precio del producto" required value="{{ $listp->precio }}">
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="name">
                    Cantidad disponible
                ...
                <input
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="dispo" name="dispo" type="number" min="1"  placeholder="Ingresa el precio del producto" required value="{{ $listp->existencia }}" }>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="name">
                    Descripcion del producto
                </label>
                <textarea class="shadow resize-none appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="descripcion" id="message1" type="text" placeholder="Escríbe su descripción" required>{{ $listp->descripcion }}</textarea>
            </div>
            <div class="mb-4">

                <label class="block text-gray-700 text-sm font-bold mb-2" for="name">
                    Elegir tú número de teléfono para contactarte <!--selected-->
                </label>
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="message1" id="message1" type="text" placeholder="--Seleccione--"required>
                    @if (count($listt) != 0)
                        @foreach ($listt as $item)
                            @if($item->id == $listp->id_telefono)
                                <option value="{{ $item->id }}" selected>{{ $item->telefono }}</option>
                            @else
            ...
            <div class="mb-4">
                ...
                <label class="inline-flex items-center mt-3"><!--checked-->
                    @if($listp->negociable)
                        <input type="checkbox" name="negociable" checked class="form-checkbox h-5 w-5 text-blue-600">
                    @else
                        <input type="checkbox" name="negociable" class="form-checkbox h-5 w-5 text-blue-600">
                    @endif
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="name">
                    Añadir Fotos(Si no desea cambiarlo dejarlo vacio)                </label>
            ...
            <div class="mx-auto">
                ...
                <a href="/myArticules" class="no-underline">
                    <button id="button"
                            class="mx-auto w-full
