...
class ProductoController extends Controller
{
    ...
    {
        if (!empty(session('id'))) {
            $list = DB::select("SELECT p.id as 'idp', p.nombre as 'nombrep',p.descripcion as 'descripcionp' FROM productos p INNER JOIN telefonos t ON t.id = p.id_telefono INNER JOIN usuarios u ON u.id = t.id_usuario WHERE u.id =" . session('id'));
